...
const chatBtnLabel = chatBtn ? chatBtn.querySelector(".btn-label") : null;
const chatSpinner = chatBtn ? chatBtn.querySelector(".btn-spinner") : null;
...
if (chatForm) {
  chatForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      chatError.textContent = "chat failed: " + (err && err.message ? err.message : err);
...
function appendMsg(role, text) {
  ...
  const prefix = role === "bot" ? "nullify &rsaquo;" : "you &rsaquo;";
  div.innerHTML = `<span class="msg-prefix">${prefix}</span>${esc(text)}`;
...
function setChatLoading(on) {
  ...
  if (chatSpinner) chatSpinner.hidden = !on;
  if (chatBtnLabel) chatBtnLabel.textContent = on ? "thinking…" : "send \u2192";
}
